...
int main(void)
{
	...
	//size in bytes of the memories hanging from GP0 and GP1 (0x8000=32KB)
	#define ON_CHIP_MEMORY_SPAN 0x8000
	...
	//asign addresses to be used for CPU and DMA
	...
	wr_dst = (char*) ADDRESS_MEMORY_DMA;
	rd_src = (char*) ADDRESS_MEMORY_DMA;
	...
	//-------------CHECKING FPGA 32kB ON-CHIP RAMs-------------//
    ...
    for(i=0; i<number_of_data_sizes; i++)//for each data size
	{
		...
		//if data is bigger than on-chip size in bytes (DMA sends half and CPU half)
		if ((data_size[i]/2)>ON_CHIP_MEMORY_SPAN)
		{
			data_in_one_operation = ON_CHIP_MEMORY_SPAN;
			operation_loops = (data_size[i]/2)/ON_CHIP_MEMORY_SPAN;
		}
		else
		{
			data_in_one_operation = data_size[i]/2;
			operation_loops = 1;
		}


		//reserve the exact memory of the data size
		//DMA will use the first half and CPU the second half
		...
		for (j=0; j<operation_loops; j++)
		{
			...
			//if (l>=2) printf("DMA WR src=%p, dst=%p, size=%d\n", (void*)(&(wr_src[j*ON_CHIP_MEMORY_SPAN])), (void*)wr_dst, data_in_one_operation);
		}
		Channel=0;
				...
				if (DmaProg){
					...
					while (Checked[Channel] == 0){ //wait DMAC to finish
						...
						if (cpu_finished == 0)
						{
							...
							cpu_finished = 1;
							//if (l>=2) printf("CPU WR src=%p, dst=%p, size=%d\n",  (void *) (&(data[data_size[i]/2 + j*ON_CHIP_MEMORY_SPAN])), (void*) FPGA_on_chip_RAM_addr_CPU, data_in_one_operation);
						}
					}
					overflow = pmu_counter_read_ns(&pmu_counter_ns);
					if (overflow == 1){printf("Cycle counter overflow!! Program ended\n\r");return 1;}
...
// Read operation (DMA)
	...
	for (j=0; j<operation_loops; j++)
	{
		...
		//if (l>=2) printf("DMA RD src=%p, dst=%p, size=%d\n",  (void*)rd_src,  (void*)(&(rd_dst[j*ON_CHIP_MEMORY_SPAN])), data_in_one_operation);
	}
		...
		if (DmaProg){
			...
			while (Checked[Channel] == 0){ //wait DMAC to finish
				...
				if (cpu_finished == 0)
				{
					...
					//if (l>=2) printf("CPU RD src=%p, dst=%p, size=%d\n",  FPGA_on_chip_RAM_addr_CPU, (void *) (&(data[data_size[i]/2 + j*ON_CHIP_MEMORY_SPAN])), data_in_one_operation);
					cpu_finished = 1;
				}
			}
			overflow = pmu_counter_read_ns(&pmu_counter_ns);
